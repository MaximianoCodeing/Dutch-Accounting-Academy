...
// migração segura: se a base de dados já existir de antes desta versão,
// a coluna is_admin ainda não existe — tenta adicionar, ignora se já lá estiver
try {
  db.exec("ALTER TABLE users ADD COLUMN is_admin INTEGER NOT NULL DEFAULT 0");
} catch (e) {
  // coluna já existe — normal em arranques seguintes
}
...
function userFromRequest(req) {
  ...
  const row = db
    .prepare("SELECT u.id, u.username, u.is_admin FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ?")
    .get(token);
  return row || null;
}
...
const routes = {
  "POST /api/register": async (req, res) => {
    ...
    send(res, 201, { token, username: name, isAdmin: false });
  },
  ...
  "POST /api/login": async (req, res) => {
    ...
    send(res, 200, { token, username: user.username, isAdmin: !!user.is_admin });
  },
  ...
  "POST /api/custom-courses": async (req, res) => {
    const user = userFromRequest(req);
    if (!user) return send(res, 401, { error: "Sessão inválida." });
    if (!user.is_admin) return send(res, 403, { error: "Só administradores podem criar cursos." });
  ...
  "DELETE /api/custom-courses": async (req, res) => {
    const user = userFromRequest(req);
    if (!user) return send(res, 401, { error: "Sessão inválida." });
    if (!user.is_admin) return send(res, 403, { error: "Só administradores podem apagar cursos." });
    const url = new URL(req.url, `http://${req.headers.host}`);
    const id = Number(url.searchParams.get("id"));
    if (!id) return send(res, 400, { error: "ID em falta." });

    const row = db.prepare("SELECT id FROM custom_courses WHERE id = ?").get(id);
  ...
  "POST /api/custom-tasks": async (req, res) => {
    const user = userFromRequest(req);
    if (!user) return send(res, 401, { error: "Sessão inválida." });
    if (!user.is_admin) return send(res, 403, { error: "Só administradores podem criar tarefas." });
  ...
  "DELETE /api/custom-tasks": async (req, res) => {
    const user = userFromRequest(req);
    if (!user) return send(res, 401, { error: "Sessão inválida." });
    if (!user.is_admin) return send(res, 403, { error: "Só administradores podem apagar tarefas." });
    const url = new URL(req.url, `http://${req.headers.host}`);
    const id = Number(url.searchParams.get("id"));
    if (!id) return send(res, 400, { error: "ID em falta." });

    const row = db.prepare("SELECT id FROM custom_tasks WHERE id = ?").get(id);
  ...
  "POST /api/custom-lessons": async (req, res) => {
    const user = userFromRequest(req);
    if (!user) return send(res, 401, { error: "Sessão inválida." });
    if (!user.is_admin) return send(res, 403, { error: "Só administradores podem adicionar lições." });
  ...
  "DELETE /api/custom-lessons": async (req, res) => {
    const user = userFromRequest(req);
    if (!user) return send(res, 401, { error: "Sessão inválida." });
    if (!user.is_admin) return send(res, 403, { error: "Só administradores podem apagar lições." });
    const url = new URL(req.url, `http://${req.headers.host}`);
    const id = Number(url.searchParams.get("id"));
    if (!id) return send(res, 400, { error: "ID em falta." });

    const row = db.prepare("SELECT id FROM custom_lessons WHERE id = ?").get(id);
  ...
  /* ---- administração: só um admin pode nomear/remover outro admin,
     confirmando sempre a própria password (não basta estar autenticado) ---- */
  "POST /api/admin/grant": async (req, res) => {
    const acting = userFromRequest(req);
    if (!acting || !acting.is_admin) return send(res, 403, { error: "Só administradores podem fazer isto." });

    const { username, password } = await readBody(req);
    const actingFull = db.prepare("SELECT * FROM users WHERE id = ?").get(acting.id);
    if (!verifyPassword(password || "", actingFull.salt, actingFull.hash))
      return send(res, 401, { error: "Palavra-passe incorreta." });

    const target = (username || "").trim().toLowerCase();
    const info = db.prepare("UPDATE users SET is_admin = 1 WHERE username = ?").run(target);
    if (info.changes === 0) return send(res, 404, { error: "Utilizador não encontrado." });
  ...
  "POST /api/admin/revoke": async (req, res) => {
    ...
    const target = (username || "").trim().toLowerCase();
    const info = db.prepare("UPDATE users SET is_admin = 0 WHERE username = ?").run(target);
    if (info.changes === 0) return send(res, 404, { error: "Utilizador não encontrado." });
