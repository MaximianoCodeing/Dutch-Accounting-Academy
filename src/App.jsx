...
const PALETTE = {
  light: {
    bg: "#F7F3EA",        // cream / ivory
    bgAlt: "#F0EADC",     // cream ligeiramente mais escuro
    panel: "#F7F3EA",     // bloco editorial, mesmo tom do fundo
    ledgerLine: "#E7DFCC",
    ink: "#12203E",       // navy quase preto — texto principal
    inkSoft: "#4A5876",   // azul/cinza discreto — texto secundário
    muted: "#8A8370",     // beige/cinza quente
    border: "#DFD6C0",    // border beige subtil
    orange: "#12203E",    // navy — cor primária (botões, ativo)
    orangeSoft: "#E9E2CF",
    stampRed: "#A94438",  // vermelho muito reduzido, só para erros
    green: "#B08D57",     // dourado/ocre — estados concluídos, destaque
    gold: "#B08D57",
  },
  dark: {
    bg: "#0E1526",        // navy muito escuro
    bgAlt: "#131C31",
    panel: "#131C31",
    ledgerLine: "#1E2A45",
    ink: "#F3EFE3",       // cream/off-white — texto principal
    inkSoft: "#C7C2AE",
    muted: "#8B93A8",
    border: "#28324A",
    orange: "#C9A45C",    // dourado — cor primária sobre navy escuro
    orangeSoft: "#2A2415",
    stampRed: "#D97563",
    green: "#C9A45C",     // dourado — estados concluídos
    gold: "#C9A45C",
  },
};
...
const STR = {
  pt: {
    nav_dashboard: "Painel", nav_courses: "Cursos", nav_company: "Modo Empresa", nav_account: "Conta", nav_certificates: "Certificados",
    ...
    form_save_task: "Guardar tarefa", edit: "Editar", edit_course: "Editar curso", edit_lesson: "Editar lição", edit_task: "Editar tarefa", form_save_edit: "Guardar edição",
    ...
    add_lesson_to_course: "Adicionar lição a este curso", form_save_lesson: "Guardar lição",
    admin_panel: "Administração", admin_panel_note: "Só administradores podem criar ou apagar cursos, tarefas e lições. Nomeia ou remove um administrador confirmando a tua própria palavra-passe.",
    admin_target_username: "Nome de utilizador do outro", admin_your_password: "A tua palavra-passe (para confirmar)",
    admin_grant: "Tornar admin", admin_revoke: "Remover admin",
    admin_granted: "Utilizador promovido a administrador.", admin_revoked: "Privilégio de administrador removido.",
  ...
  en: {
    nav_dashboard: "Dashboard", nav_courses: "Courses", nav_company: "Company Mode", nav_account: "Account", nav_certificates: "Certificates",
    ...
    form_save_task: "Save task", edit: "Edit", edit_course: "Edit course", edit_lesson: "Edit lesson", edit_task: "Edit task", form_save_edit: "Save changes",
    ...
    add_lesson_to_course: "Add lesson to this course", form_save_lesson: "Save lesson",
    admin_panel: "Administration", admin_panel_note: "Only administrators can create or delete courses, tasks and lessons. Grant or revoke admin by confirming your own password.",
    admin_target_username: "Other user's username", admin_your_password: "Your password (to confirm)",
    admin_grant: "Make admin", admin_revoke: "Remove admin",
    admin_granted: "User promoted to administrator.", admin_revoked: "Administrator privilege removed.",
  ...
  nl: {
    nav_dashboard: "Dashboard", nav_courses: "Cursussen", nav_company: "Bedrijfsmodus", nav_account: "Account", nav_certificates: "Certificaten",
    ...
    form_save_task: "Taak opslaan", edit: "Bewerken", edit_course: "Cursus bewerken", edit_lesson: "Les bewerken", edit_task: "Taak bewerken", form_save_edit: "Wijzigingen opslaan",
    ...
    add_lesson_to_course: "Les toevoegen aan deze cursus", form_save_lesson: "Les opslaan",
    admin_panel: "Beheer", admin_panel_note: "Alleen beheerders kunnen cursussen, taken en lessen aanmaken of verwijderen. Ken beheerder toe of verwijder door je eigen wachtwoord te bevestigen.",
    admin_target_username: "Gebruikersnaam van de ander", admin_your_password: "Jouw wachtwoord (ter bevestiging)",
    admin_grant: "Beheerder maken", admin_revoke: "Beheerder verwijderen",
    admin_granted: "Gebruiker gepromoveerd tot beheerder.", admin_revoked: "Beheerdersrechten verwijderd.",
...
function normalizeCustomLesson(raw) {
  return {
    custom: true,
    customLessonId: raw.id,
    ownerName: raw.ownerName,
    title: wrapLang(raw.title),
    theory: wrapLang(raw.theory),
    flashcard: { front: wrapLang(raw.flashcard?.front), back: wrapLang(raw.flashcard?.back) },
    quizzes: [{
      q: wrapLang(raw.quiz?.q),
      options: (raw.quiz?.options || ["", "", ""]).map(wrapLang),
      answer: raw.quiz?.answer ?? 0,
    }],
  };
}
...
function Stamp({ label, color, earned, size = "md", pal }) {
  ...
  return (
    <div style={{
      ...
      color: earned ? stampColor : pal.muted, fontFamily: "'Fraunces', serif", fontWeight: 700,
...
function LedgerPanel({ children, pal, style, className = "" }) {
  return (
    <div style={{
      background: pal.panel, border: `1px solid ${pal.border}`, borderRadius: 2,
      ...style,
...
function XPBar({ value, max, pal }) {
  ...
  return (
    <div className="w-full h-[3px]" style={{ background: pal.border }}>
      <div className="h-full transition-all" style={{ width: `${pct}%`, background: pal.gold }} />
    </div>
  );
}
...
function SectionEyebrow({ children, pal }) {
  return (
    <div className="text-xs font-semibold tracking-widest uppercase mb-2"
      style={{ color: pal.orange, fontFamily: "'Fraunces', serif", letterSpacing: "0.12em" }}>
      {children}
    </div>
  );
}
...
function VocabularyPage({ t, pal, lang }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold" style={{ color: pal.ink, fontFamily: "'Fraunces', serif" }}>{t.vocabulary_title}</h1>
        ...
        {filtered.map((v, i) => (
          <LedgerPanel key={i} pal={pal}>
            <div className="flex items-baseline justify-between gap-2 mb-1.5">
              <span className="text-base font-semibold" style={{ color: pal.orange, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>{v.term}</span>
            </div>
...
function DashboardPage({ t, pal, lang, progress, level, careerIndex, missions, onContinue }) {
  ...
  const metrics = [
    ...
    { label: t.xp, value: String(progress.xp) },
    { label: t.streak, value: String(progress.streak) },
    { label: t.coins, value: String(progress.coins) },
  ];

  return (
    <div>
      <div className="text-xs uppercase tracking-widest mb-3" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_dashboard}</div>
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.dashboard_title}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.dashboard_sub}</p>

      {/* faixa de métricas */}
      <div className="grid grid-cols-2 sm:grid-cols-4 mb-12" style={{ borderTop: `1px solid ${pal.border}`, borderBottom: `1px solid ${pal.border}` }}>
        {metrics.map((m, i) => (
          <div key={i} className="py-5 px-1 sm:px-4" style={{ borderLeft: i > 0 ? `1px solid ${pal.border}` : "none" }}>
            <div className="text-3xl" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{m.value}</div>
            <div className="text-[11px] uppercase tracking-wider mt-1.5" style={{ color: pal.muted, letterSpacing: "0.08em" }}>{m.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mb-12">
        ...
        <div className="mt-3" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 20 }}>
          <div className="text-xl mb-1" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{nextCourse.title[lang]}</div>
          <div className="text-sm mb-5" style={{ color: pal.inkSoft }}>
            {nextLessonIdx >= 0 ? nextCourse.lessons[nextLessonIdx].title[lang] : nextCourse.lessons[0].title[lang]}
          </div>
          <XPBar value={doneLessons} max={totalLessons} pal={pal} />
          <div className="flex items-center justify-between mt-2.5">
            <span className="text-xs" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
              {doneLessons} / {totalLessons} {t.of_lessons}
            </span>
            <button
              onClick={() => onContinue(nextCourse.id, nextLessonIdx >= 0 ? nextLessonIdx : 0)}
              className="text-sm font-medium"
              style={{ color: pal.ink, fontFamily: "'Inter', sans-serif" }}>
              {t.continue} →
            </button>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <div className="mt-3 space-y-4" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 16 }}>
            {missions.daily.map((m, i) => (
              <div key={i}>
                <div className="flex justify-between text-sm mb-1.5" style={{ color: pal.ink }}>
                  <span>{m.title[lang]}</span>
                  <span style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>{m.progress}/{m.total}</span>
                </div>
        ...
        <div>
          ...
          <div className="mt-3" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 16 }}>
            <div className="text-lg mb-1" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>
              {CAREER_LEVELS[careerIndex][lang]}
            </div>
...
function FlashcardWidget({ card, lang, t, pal, onFlip }) {
  ...
  return (
    <div>
      ...
      <div onClick={flip}
        className="cursor-pointer rounded-sm p-6 min-h-[110px] flex items-center justify-center text-center transition-all select-none"
      ...
      <button onClick={flip} className="mt-2 text-xs font-medium uppercase tracking-wide"
        style={{ color: pal.orange, fontFamily: "'Fraunces', serif" }}>{t.flip} ↻</button>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="flex items-center gap-2 mt-3">
        <button disabled={selected === null}
          ...
          className="px-4 py-1.5 rounded-sm text-sm font-medium disabled:opacity-40"
          style={{ background: pal.ink, color: pal.panel, fontFamily: "'Fraunces', serif" }}>
          {t.check_answer}
        </button>
        {checked && selected === quiz.answer && !isLast && (
          <button onClick={goNext}
            className="px-4 py-1.5 rounded-sm text-sm font-medium"
            style={{ background: pal.orange, color: "#fff", fontFamily: "'Fraunces', serif" }}>
            {t.next_exercise}
          </button>
        )}
      </div>
      ...
      {checked && quiz.explain && (
        <div className="mt-2 text-xs leading-relaxed px-3 py-2.5 rounded-sm"
          style={{ background: pal.orangeSoft, color: pal.inkSoft }}>
          {quiz.explain[lang]}
        </div>
      )}
      {alreadyDone && !checked && (
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="text-xs uppercase tracking-widest" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_courses}</div>
        {account.user && (
          <button onClick={() => setShowCreate(true)} className="text-xs font-medium" style={{ color: pal.ink }}>
            + {t.new_course}
          </button>
        )}
      </div>
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.courses_title}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.courses_sub}</p>
          ...
          return (
            <div key={c.id} style={{ borderBottom: `1px solid ${pal.border}` }}>
              <div className="flex items-center gap-3 py-5">
                <button className="flex-1 flex items-center gap-4 text-left min-w-0" onClick={() => setOpenCourse(isOpen ? null : c.id)}>
                  <span className="text-sm flex-shrink-0" style={{ color: pal.gold, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
                    {String(ci + 1).padStart(2, "0")}
                  </span>
                  <div className="min-w-0">
                    <div className="text-base flex items-center gap-2 flex-wrap" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>
                      {c.title[lang]}
                      {c.custom && (
                        <span className="text-[10px] px-1 py-0.5" style={{ border: `1px solid ${pal.border}`, color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace", fontWeight: 400 }}>
                          {t.custom_tag}
                        </span>
                      )}
                    </div>
                    <div className="text-xs mt-0.5" style={{ color: pal.muted, fontFamily: "'Inter', sans-serif" }}>{progressPct}% · {doneCount}/{c.lessons.length} {t.of_lessons}</div>
                  </div>
                </button>
                {c.custom && account.user === c.ownerName && (
                  <>
                    <button onClick={() => setEditCourse(c)} aria-label={t.edit} className="text-xs font-medium flex-shrink-0" style={{ color: pal.gold }}>
                      {t.edit}
                    </button>
                ...
                <span className="flex-shrink-0" style={{ color: pal.muted }}>{isOpen ? "−" : "+"}</span>
              </div>
              <div className="mt-3"><XPBar value={doneCount} max={c.lessons.length} pal={pal} /></div>

              {isOpen && (
                <div className="mt-2 pb-8 grid md:grid-cols-3 gap-6">
                  {/* Lesson list */}
                  <div className="space-y-1.5">
                      ...
                      return (
                        <div key={i} className="flex items-center gap-1">
                          <button
                            ...
                            className="flex-1 flex items-center gap-2 text-left text-sm px-2.5 py-2 rounded-sm transition-colors min-w-0"
                            ...
                            {l.custom && (
                              <span className="text-[9px] px-1 py-0.5 rounded-sm flex-shrink-0" style={{ background: pal.bgAlt, color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
                                {t.custom_tag}
                              </span>
                            )}
                          </button>
                          {l.custom && account.user === l.ownerName && (
                            <button onClick={() => setEditLesson(l)} aria-label={t.edit} className="p-1 flex-shrink-0 text-[10px] font-medium" style={{ color: pal.gold }}>
                              {t.edit}
                            </button>
                          )}
                          {l.custom && account.user === l.ownerName && (
                            <button
                              onClick={async () => {
                                if (!window.confirm(t.confirm_delete)) return;
                                try {
                                  await api.deleteCustomLesson(l.customLessonId);
                                  c.lessons.splice(i, 1);
                                  onRefreshCustom();
                                } catch (e) { window.alert(e.message); }
                              }}
                              aria-label={t.delete} className="p-1 flex-shrink-0" style={{ color: pal.muted }}>
                              <X size={13} />
                            </button>
                          )}
                        </div>
                    ...
                    {account.user && (
                      <button onClick={() => setAddLessonTo(c.id)}
                        className="w-full text-left text-xs font-medium px-2.5 py-2 mt-1"
                        style={{ color: pal.orange, fontFamily: "'Fraunces', serif" }}>
                        + {t.add_lesson_to_course}
                      </button>
                    )}
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-5">
                    <div>
                      <div className="text-sm font-semibold mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif" }}>
      ...
      {editCourse && (
        <CreateCourseModal t={t} pal={pal} lang={lang} editCourse={editCourse}
          onClose={() => setEditCourse(null)}
          onCreated={() => { setEditCourse(null); onRefreshCustom(); }}
        />
      )}

      {addLessonTo && (
        <AddLessonModal t={t} pal={pal} lang={lang} courseId={addLessonTo}
          onClose={() => setAddLessonTo(null)}
          onCreated={() => { setAddLessonTo(null); onRefreshCustom(); }}
        />
      )}

      {editLesson && (
        <AddLessonModal t={t} pal={pal} lang={lang} courseId={null} editLesson={editLesson}
          onClose={() => setEditLesson(null)}
          onCreated={() => { setEditLesson(null); onRefreshCustom(); }}
        />
      )}
    </div>
  );
...
function AddLessonModal({ t, pal, lang, courseId, onClose, onCreated, editLesson }) {
  const isEdit = !!editLesson;
  const [sourceLang, setSourceLang] = useState(lang);
  const [title, setTitle] = useState(editLesson?.title?.[lang] || "");
  const [theory, setTheory] = useState(editLesson?.theory?.[lang] || "");
  const [flashFront, setFlashFront] = useState(editLesson?.flashcard?.front?.[lang] || "");
  const [flashBack, setFlashBack] = useState(editLesson?.flashcard?.back?.[lang] || "");
  const [q, setQ] = useState(editLesson?.quizzes?.[0]?.q?.[lang] || "");
  const [opt1, setOpt1] = useState(editLesson?.quizzes?.[0]?.options?.[0]?.[lang] || "");
  const [opt2, setOpt2] = useState(editLesson?.quizzes?.[0]?.options?.[1]?.[lang] || "");
  const [opt3, setOpt3] = useState(editLesson?.quizzes?.[0]?.options?.[2]?.[lang] || "");
  const [answer, setAnswer] = useState(editLesson?.quizzes?.[0]?.answer ?? 0);
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        title: title.trim(),
        theory: theory.trim(),
        flashcard: { front: flashFront.trim(), back: flashBack.trim() },
        quiz: { q: q.trim(), options: [opt1.trim(), opt2.trim(), opt3.trim()].filter(Boolean), answer },
      };
      if (isEdit) await api.updateCustomLesson(editLesson.customLessonId, payload);
      else await api.createCustomLesson(payload);
      onCreated();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div onClick={(e) => e.stopPropagation()}
        className="w-full sm:max-w-lg max-h-[88vh] overflow-y-auto rounded-t-2xl sm:rounded-sm p-5"
        style={{ background: pal.panel, border: `1px solid ${pal.border}` }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold" style={{ color: pal.ink, fontFamily: "'Fraunces', serif" }}>{isEdit ? t.edit_lesson : t.add_lesson_to_course}</h2>
        ...
        <div className="flex gap-2 mb-4">
          {["pt", "en", "nl"].map((l) => (
            <button key={l} onClick={() => setSourceLang(l)}
              ...
              style={{ background: sourceLang === l ? pal.orange : pal.bgAlt, color: sourceLang === l ? "#fff" : pal.inkSoft, fontFamily: "'Fraunces', serif" }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-2">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t.form_lesson_title}
            className="w-full px-3 py-2 rounded-sm text-sm outline-none" style={inputStyle} />
          <textarea value={theory} onChange={(e) => setTheory(e.target.value)} placeholder={t.form_theory} rows={5}
            className="w-full px-3 py-2 rounded-sm text-sm outline-none resize-y" style={inputStyle} />
          <div className="grid sm:grid-cols-2 gap-2">
            <input value={flashFront} onChange={(e) => setFlashFront(e.target.value)} placeholder={t.form_flash_front}
              className="w-full px-3 py-2 rounded-sm text-sm outline-none" style={inputStyle} />
            <input value={flashBack} onChange={(e) => setFlashBack(e.target.value)} placeholder={t.form_flash_back}
              className="w-full px-3 py-2 rounded-sm text-sm outline-none" style={inputStyle} />
          </div>
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t.form_quiz_q}
            className="w-full px-3 py-2 rounded-sm text-sm outline-none" style={inputStyle} />
          {[["opt1", opt1, setOpt1], ["opt2", opt2, setOpt2], ["opt3", opt3, setOpt3]].map(([key, val, setter], oi) => (
            <div key={key} className="flex items-center gap-2">
              <button onClick={() => setAnswer(oi)}
                className="w-5 h-5 rounded-full flex-shrink-0"
                style={{ border: `2px solid ${answer === oi ? pal.green : pal.border}`, background: answer === oi ? pal.green : "transparent" }} />
              <input value={val} onChange={(e) => setter(e.target.value)} placeholder={`${t.form_option} ${oi + 1}`}
                className="flex-1 px-3 py-1.5 rounded-sm text-sm outline-none" style={inputStyle} />
            </div>
          ))}
        ...
        <button onClick={submit} disabled={busy}
          className="mt-4 w-full px-4 py-2.5 rounded-sm text-sm font-medium disabled:opacity-50"
          style={{ background: pal.orange, color: "#fff", fontFamily: "'Fraunces', serif" }}>
          {busy ? t.form_translating : isEdit ? t.form_save_edit : t.form_save_lesson}
        </button>
      </div>
    </div>
  );
...
function CreateCourseModal({ t, pal, lang, onClose, onCreated, editCourse }) {
  const isEdit = !!editCourse;
  const [title, setTitle] = useState(editCourse?.title?.[lang] || "");
  const [icon, setIcon] = useState(editCourse?.iconName || "BookOpen");
  const [sourceLang, setSourceLang] = useState(lang);
  const [lessons, setLessons] = useState(
    editCourse?.lessons?.length
      ? editCourse.lessons.map((l) => ({
          title: l.title?.[lang] || "", theory: l.theory?.[lang] || "",
          flashFront: l.flashcard?.front?.[lang] || "", flashBack: l.flashcard?.back?.[lang] || "",
          q: l.quizzes?.[0]?.q?.[lang] || "",
          opt1: l.quizzes?.[0]?.options?.[0]?.[lang] || "", opt2: l.quizzes?.[0]?.options?.[1]?.[lang] || "", opt3: l.quizzes?.[0]?.options?.[2]?.[lang] || "",
          answer: l.quizzes?.[0]?.answer ?? 0,
        }))
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (isEdit) await api.updateCustomCourse(editCourse.dbId, course);
      else await api.createCustomCourse(course);
      onCreated();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end sm:items-center justify-center p-0 sm:p-6"
      style={{ background: "rgba(0,0,0,0.5)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()}
        className="w-full sm:max-w-xl max-h-[88vh] overflow-y-auto rounded-t-2xl sm:rounded-sm p-5"
        style={{ background: pal.panel, border: `1px solid ${pal.border}` }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold" style={{ color: pal.ink, fontFamily: "'Fraunces', serif" }}>{isEdit ? t.edit_course : t.new_course}</h2>
          <button onClick={onClose} style={{ color: pal.muted }}><X size={18} /></button>
        </div>
        ...
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t.form_course_title}
          className="w-full px-3 py-2 rounded-sm text-sm outline-none mb-3" style={inputStyle} />
        ...
        <div className="space-y-4">
          {lessons.map((l, i) => (
            <div key={i} className="p-3 rounded-sm space-y-2" style={{ background: pal.bgAlt, border: `1px solid ${pal.border}` }}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>{t.lessons} {i + 1}</span>
                {lessons.length > 1 && (
        ...
        <button onClick={addLesson} className="mt-3 text-xs font-medium" style={{ color: pal.orange, fontFamily: "'Fraunces', serif" }}>
          + {t.form_add_lesson}
        </button>
        ...
        <button onClick={submit} disabled={busy}
          className="mt-4 w-full px-4 py-2.5 rounded-sm text-sm font-medium disabled:opacity-50"
          style={{ background: pal.orange, color: "#fff", fontFamily: "'Fraunces', serif" }}>
          {busy ? t.form_translating : isEdit ? t.form_save_edit : t.form_save_course}
        </button>
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div className="space-y-2">
        {rows.map((row, i) => {
          ...
          return (
            <div key={i} className="flex flex-wrap items-center gap-2 p-2 rounded-sm" style={{ border: `1.5px solid ${borderColor}`, background: pal.bgAlt }}>
              <select
                value={row.account}
                onChange={(e) => updateRow(i, { account: e.target.value })}
                className="flex-1 min-w-[140px] px-2 py-1.5 rounded-sm text-sm"
              ...
              <div className="flex rounded-sm overflow-hidden flex-shrink-0" style={{ border: `1px solid ${pal.border}` }}>
              ...
              <input
                ...
                className="w-24 px-2 py-1.5 rounded-sm text-sm flex-shrink-0"
                style={{ background: pal.panel, color: pal.ink, border: `1px solid ${pal.border}`, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}
              />
      ...
      <div className="flex items-center gap-2 mt-3">
        ...
        <button
          onClick={reset}
          className="px-4 py-1.5 rounded-sm text-sm font-medium"
          style={{ border: `1.5px solid ${pal.border}`, color: pal.inkSoft, fontFamily: "'Fraunces', serif" }}>
          {t.journal_retry}
        </button>
      </div>
  ...
  return (
    <div className="mb-4">
      <div className="flex items-baseline justify-between mb-2">
        ...
        <span className="text-xs" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>{index + 1}/{total}</span>
      </div>
      ...
      <div className="flex items-center gap-2 mt-3">
        <button
          ...
          disabled={index === 0}
          className="px-3 py-1.5 rounded-sm text-xs font-medium disabled:opacity-30"
          style={{ border: `1.5px solid ${pal.border}`, color: pal.inkSoft, fontFamily: "'Fraunces', serif" }}>
          ← {t.journal_prev}
        </button>
        <button
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>€</span>
        <input
          type="number"
          value={value}
          onChange={(e) => { setValue(e.target.value); setChecked(false); }}
          placeholder={t.calc_answer}
          className="w-32 px-2 py-1.5 rounded-sm text-sm"
          style={{
...
function TaskModal({ task, lang, t, pal, onClose, onComplete, done }) {
  ...
  return (
    <div key={task.id} className="fixed inset-0 z-30 flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div onClick={(e) => e.stopPropagation()}
        className="w-full sm:max-w-lg max-h-[85vh] overflow-y-auto rounded-t-2xl sm:rounded-sm p-5"
        style={{ background: pal.panel, border: `1px solid ${pal.border}` }}>
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <h2 className="text-base font-semibold leading-snug"
              style={{ color: pal.ink, fontFamily: "'Fraunces', serif" }}>{task.title[lang]}</h2>
          </div>
        ...
        <p className="text-sm leading-relaxed mb-5 whitespace-pre-line" style={{ color: pal.inkSoft, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace", fontSize: "0.8rem", lineHeight: 1.7 }}>{task.brief[lang]}</p>
        ...
        {needsExercise ? (
          ...
          <div className="text-xs mb-4 px-3 py-2.5 rounded-sm" style={{ background: pal.orangeSoft, color: pal.inkSoft }}>
            {t.demo_note}
          </div>
        )}

        <div className="flex items-center justify-between gap-3">
          <span className="text-xs" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>+{task.xp} {t.xp}</span>
          <button onClick={onComplete} disabled={!canComplete}
            className="px-4 py-2 rounded-sm text-sm font-medium disabled:opacity-40"
            style={{ background: done ? pal.green : pal.orange, color: "#fff", fontFamily: "'Fraunces', serif" }}>
...
function CompanyPage({ t, pal, lang, progress, level, careerIndex, onTaskComplete, account, onRefreshCustom }) {
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="text-xs uppercase tracking-widest" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_company}</div>
        {account.user && (
          <button onClick={() => setShowCreate(true)} className="text-xs font-medium" style={{ color: pal.ink }}>
            + {t.new_task}
          </button>
      ...
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.company_title}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.company_sub}</p>

      <div className="grid md:grid-cols-3 gap-12">
        <div className="md:col-span-2">
          <SectionEyebrow pal={pal}>{t.daily_tasks}</SectionEyebrow>
          <div className="mt-2" style={{ borderTop: `1px solid ${pal.border}` }}>
            {COMPANY_TASKS.map((task, i) => {
              const status = statusOf(task);
              const locked = status === "locked";
              const done = status === "done";
              return (
                <div key={task.id} className="flex items-center gap-4 py-4" style={{ borderBottom: `1px solid ${pal.border}`, opacity: locked ? 0.5 : 1 }}>
                  <span className="text-xs flex-shrink-0" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm flex items-center gap-2 flex-wrap" style={{ color: pal.ink }}>
                      {task.title[lang]}
                      {task.custom && (
                        <span className="text-[10px] px-1 py-0.5" style={{ border: `1px solid ${pal.border}`, color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
                          {t.custom_tag}
                        </span>
                      )}
                    </div>
                  </div>
                  <span className="text-xs flex-shrink-0" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>+{task.xp} {t.xp}</span>
                  {task.custom && account.user === task.ownerName && (
                    <button onClick={() => setEditTask(task)} aria-label={t.edit} className="text-xs font-medium flex-shrink-0" style={{ color: pal.gold }}>
                      {t.edit}
                    </button>
                  )}
                  {task.custom && account.user === task.ownerName && (
                  ...
                  {done ? (
                    <button onClick={() => setOpenTask(task)} className="text-xs font-medium flex-shrink-0" style={{ color: pal.gold }}>
                      {t.completed} →
                    </button>
                  ) : (
                    <button disabled={locked} onClick={() => setOpenTask(task)} className="text-xs font-medium flex-shrink-0"
                      style={{ color: locked ? pal.muted : pal.ink }}>
        ...
        <div>
          ...
          <div className="relative mt-3 pl-5" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 20 }}>
            <div className="absolute left-[3px] top-6 bottom-1 w-px" style={{ background: pal.border }} />
            <div className="space-y-6">
              {CAREER_LEVELS.slice().reverse().map((lvl, ri) => {
                ...
                return (
                  <div key={lvl.key} className="relative">
                    <div className="absolute -left-5 top-1.5 w-[7px] h-[7px] rounded-full flex-shrink-0"
                      style={{ background: state === "locked" ? pal.border : pal.gold }} />
                    <div className="text-sm" style={{ color: state === "locked" ? pal.muted : pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>
                      {lvl[lang]}
                    </div>
      ...
      {editTask && (
        <CreateTaskModal t={t} pal={pal} lang={lang} editTask={editTask}
          onClose={() => setEditTask(null)}
          onCreated={() => { setEditTask(null); onRefreshCustom(); }}
        />
      )}
    </div>
  );
...
function CreateTaskModal({ t, pal, lang, onClose, onCreated, editTask }) {
  const isEdit = !!editTask;
  const [title, setTitle] = useState(editTask?.title?.[lang] || "");
  const [xp, setXp] = useState(String(editTask?.xp || "20"));
  const [icon, setIcon] = useState(editTask?.iconName || "FileText");
  const [sourceLang, setSourceLang] = useState(lang);
  const [brief, setBrief] = useState(editTask?.brief?.[lang] || "");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const payload = { title: title.trim(), xp: Number(xp) || 10, icon, sourceLang, brief: brief.trim() };
      if (isEdit) await api.updateCustomTask(editTask.dbId, payload);
      else await api.createCustomTask(payload);
      onCreated();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end sm:items-center justify-center p-0 sm:p-6"
      style={{ background: "rgba(0,0,0,0.5)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()}
        className="w-full sm:max-w-md max-h-[85vh] overflow-y-auto rounded-t-2xl sm:rounded-sm p-5"
        style={{ background: pal.panel, border: `1px solid ${pal.border}` }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold" style={{ color: pal.ink, fontFamily: "'Fraunces', serif" }}>{isEdit ? t.edit_task : t.new_task}</h2>
          <button onClick={onClose} style={{ color: pal.muted }}><X size={18} /></button>
        </div>
        ...
        <button onClick={submit} disabled={busy}
          className="mt-4 w-full px-4 py-2.5 rounded-sm text-sm font-medium disabled:opacity-50"
          style={{ background: pal.orange, color: "#fff", fontFamily: "'Fraunces', serif" }}>
          {busy ? t.form_translating : isEdit ? t.form_save_edit : t.form_save_task}
        </button>
      </div>
    </div>
  );
...
function MissionsPage({ t, pal, lang, missions }) {
  ...
  return (
    <div>
      <div className="text-xs uppercase tracking-widest mb-3" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_missions}</div>
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.missions_title}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.missions_sub}</p>
      <div className="grid md:grid-cols-3 gap-10">
        {groups.map((g) => (
          <div key={g.key}>
            <div className="text-sm mb-4" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{g.label}</div>
            <div className="space-y-5" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 18 }}>
              {g.items.map((m, i) => (
                <div key={i}>
                  <div className="text-sm mb-1.5" style={{ color: pal.ink }}>{m.title[lang]}</div>
                  <XPBar value={m.progress} max={m.total} pal={pal} />
                  <div className="flex justify-between text-xs mt-1.5" style={{ color: pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
                    <span>{m.progress}/{m.total}</span><span>+{m.xp} XP · +{m.coins} ƒ</span>
                  </div>
                </div>
              ))}
            </div>
...
function AccountPanel({ t, pal, account }) {
  ...
  if (user) {
    return (
      <div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-3" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 16 }}>
          <div className="min-w-0">
            <div className="text-sm" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{user}</div>
            <div className="text-xs mt-0.5" style={{ color: syncState === "error" ? pal.stampRed : pal.gold }}>
              {syncState === "saving" ? t.sync_saving : syncState === "error" ? t.sync_error : t.sync_ok}
            </div>
          </div>
          <button onClick={logout} className="text-xs font-medium" style={{ color: pal.inkSoft }}>
            {t.logout}
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      <SectionEyebrow pal={pal}>{t.account}</SectionEyebrow>
      <div className="mt-3" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 16 }}>
        <p className="text-sm leading-relaxed mb-5" style={{ color: pal.inkSoft }}>{t.account_note}</p>

        <div className="flex gap-6 mb-5">
          {[["login", t.login], ["register", t.register]].map(([key, label]) => (
            <button key={key} onClick={() => setMode(key)} className="text-sm"
              style={{ color: mode === key ? pal.ink : pal.muted, fontWeight: mode === key ? 600 : 400, borderBottom: mode === key ? `1.5px solid ${pal.gold}` : "1.5px solid transparent", paddingBottom: 2 }}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4 max-w-sm">
          <div>
            <label className="block text-[11px] uppercase tracking-wider mb-1.5" style={{ color: pal.muted, letterSpacing: "0.08em" }}>{t.username}</label>
            <input value={username} onChange={(e) => setUsername(e.target.value)}
              autoComplete="username"
              className="w-full px-0 py-2 text-sm outline-none bg-transparent"
              style={{ borderBottom: `1px solid ${pal.border}`, color: pal.ink }} />
          </div>
          <div>
            <label className="block text-[11px] uppercase tracking-wider mb-1.5" style={{ color: pal.muted, letterSpacing: "0.08em" }}>{t.password}</label>
        ...
        {error && <div className="mt-3 text-xs" style={{ color: pal.stampRed }}>{error}</div>}

        <button onClick={submit} disabled={busy || !username || !password}
          className="mt-6 px-5 py-2.5 text-sm font-medium disabled:opacity-40"
          style={{ background: pal.ink, color: pal.panel, fontFamily: "'Inter', sans-serif" }}>
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center p-4 print:p-0" style={{ background: "rgba(0,0,0,0.55)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="print:shadow-none"
        style={{
          background: "#F7F3EA", color: "#12203E", width: "100%", maxWidth: 640,
          border: "1.5px solid #12203E", borderRadius: 2, padding: "56px 44px", position: "relative",
        }}>
        <button onClick={onClose} className="print:hidden absolute top-3 right-3" style={{ color: "#8A8370" }}>
          <X size={20} />
        </button>
        <div className="text-center">
          <div className="w-11 h-11 mx-auto mb-6 flex items-center justify-center" style={{ border: "1.5px solid #12203E" }}>
            <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 15 }}>DA</span>
          </div>
          <div className="text-xs uppercase tracking-widest mb-1" style={{ color: "#B08D57", fontFamily: "'Inter', sans-serif", letterSpacing: "0.18em", fontWeight: 600 }}>
            Dutch Accounting Academy
          </div>
          <h1 className="text-2xl mb-8" style={{ fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.certificate_title}</h1>
          <div className="text-sm mb-1" style={{ color: "#8A8370" }}>{t.certificate_awarded_to}</div>
          <div className="text-xl mb-8" style={{ fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{holderName}</div>
          <div className="text-sm mb-1" style={{ color: "#8A8370" }}>{t.certificate_for}</div>
          <div className="text-lg mb-10" style={{ fontFamily: "'Fraunces', serif" }}>{cert.title}</div>
          <div className="text-xs" style={{ color: "#8A8370" }}>
            {t.certificate_date}: {new Date().toLocaleDateString(lang === "pt" ? "pt-PT" : lang === "nl" ? "nl-NL" : "en-GB")}
          </div>
        </div>
        <button onClick={() => window.print()} className="print:hidden mt-10 w-full px-4 py-2.5 text-sm font-medium"
          style={{ background: "#12203E", color: "#F7F3EA", fontFamily: "'Inter', sans-serif" }}>
          {t.certificate_print}
        </button>
      </div>
    </div>
...
function CertificatePanel({ t, pal, lang, progress, account }) {
  ...
  return (
    <div>
      ...
      <div className="mt-3" style={{ borderTop: `1px solid ${pal.border}` }}>
        {certs.map((c) => (
          <div key={c.key} className="flex items-center justify-between py-3" style={{ borderBottom: `1px solid ${pal.border}`, opacity: c.earned ? 1 : 0.5 }}>
            <span className="text-sm truncate" style={{ color: pal.ink, fontFamily: c.isFinal ? "'Fraunces', serif" : "'Inter', sans-serif", fontWeight: c.isFinal ? 600 : 400 }}>{c.title}</span>
            {c.earned ? (
              <button onClick={() => setOpenCert(c)} className="text-xs font-medium flex-shrink-0" style={{ color: pal.gold }}>
                {t.view_certificate} →
              </button>
            ) : (
...
function ProfilePage({ t, pal, lang, badges, onReset }) {
  return (
    <div>
      <div className="text-xs uppercase tracking-widest mb-3" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_profile}</div>
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.profile_title}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.profile_sub}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-6 mt-4 pt-5" style={{ borderTop: `1px solid ${pal.border}` }}>
        {badges.map((b) => (
      ...
      <div className="mt-14">
        ...
        <div className="mt-3" style={{ borderTop: `1px solid ${pal.border}`, paddingTop: 16 }}>
          ...
          <button
            ...
            className="text-sm font-medium" style={{ color: pal.stampRed }}>
            {t.reset_progress}
          </button>
        </div>
      </div>
...
function AccountPage({ t, pal, account }) {
  return (
    <div>
      <div className="text-xs uppercase tracking-widest mb-3" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_account}</div>
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.nav_account}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.account_note}</p>
      <AccountPanel t={t} pal={pal} account={account} />
    </div>
  );
}

function CertificatesPage({ t, pal, lang, progress, account }) {
  return (
    <div>
      <div className="text-xs uppercase tracking-widest mb-3" style={{ color: pal.gold, fontWeight: 600, letterSpacing: "0.14em" }}>{t.nav_certificates}</div>
      <h1 className="text-3xl mb-2" style={{ color: pal.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>{t.nav_certificates}</h1>
      <p className="text-sm mb-10" style={{ color: pal.inkSoft }}>{t.certificates_sub || ""}</p>
...
const LANDING_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { background: var(--white); color: var(--ink); font-family: 'Inter', sans-serif; overflow-x: hidden; -webkit-font-smoothing: antialiased; }
  h1, h2, h3, .quote, .nav-logo, .stat .n, .feature-cell h3, .module-card h3 { font-family: 'Fraunces', serif; }
  .wrap { max-width: 1100px; margin: 0 auto; padding: 0 24px; }

  /* ---- nav ---- */
  nav { position: sticky; top: 0; z-index: 30; backdrop-filter: blur(20px); background: rgba(247,243,234,0.82); border-bottom: 1px solid var(--line); }
  ...
  .nav-logo { font-size: 14px; font-weight: 600; letter-spacing: -0.01em; }
  .nav-links { display: flex; gap: 32px; font-size: 12.5px; color: var(--ink-soft); }
  .nav-links a { text-decoration: none; color: inherit; }
  .nav-cta { font-size: 12.5px; font-weight: 500; color: var(--ink); text-decoration: none; }

  /* ---- hero ---- */
  .hero { text-align: center; padding: 96px 24px 64px; }
  .eyebrow { font-size: 19px; font-weight: 600; color: var(--ink-soft); margin-bottom: 8px; }
  .hero h1 {
    font-size: clamp(40px, 7vw, 88px); font-weight: 700; letter-spacing: -0.03em; line-height: 1.04;
    background: linear-gradient(180deg, #12203E 0%, #12203E 55%, #4A5876 100%);
    -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent;
  }
  .hero p.sub { font-size: clamp(18px, 2.4vw, 24px); color: var(--ink-soft); margin: 18px auto 0; max-width: 640px; font-weight: 400; }
  .hero-ctas { display: flex; gap: 18px; justify-content: center; margin-top: 34px; flex-wrap: wrap; }
  .btn { font-size: 15px; font-weight: 500; padding: 11px 24px; border-radius: 999px; text-decoration: none; display: inline-block; transition: transform .15s ease, background .15s ease; }
  .btn-primary { background: var(--ink); color: var(--white); }
  .btn-primary:hover { background: var(--blue-dark); transform: translateY(-1px); }
  .btn-ghost { color: var(--gold); }
  .btn-ghost:hover { text-decoration: underline; }

  /* ---- hero visual: product-ish mock ---- */
  .hero-visual {
    max-width: 980px; margin: 56px auto 0; border-radius: 28px; overflow: hidden;
    background: linear-gradient(135deg, #0B1526 0%, #12203E 60%, #3A2E1C 100%); padding: 56px 40px;
    box-shadow: 0 40px 80px rgba(0,0,0,0.18);
  }
  .hero-visual .tag { display: inline-block; background: rgba(176,141,87,0.22); color: #E9DFC8; font-size: 12px; font-weight: 600; padding: 5px 12px; border-radius: 999px; margin-bottom: 20px; }
  .hero-visual .stat-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; text-align: left; }
  .hero-visual .stat .n { font-size: clamp(30px, 4vw, 46px); font-weight: 700; color: #fff; letter-spacing: -0.02em; }
  .hero-visual .stat .l { font-size: 13px; color: rgba(255,255,255,0.72); margin-top: 6px; }

  /* ---- section ---- */
  .section { padding: 100px 24px; text-align: center; }
  .section-dark { background: #0E1526; color: #F3EFE3; }
  .section h2 { font-size: clamp(32px, 5vw, 56px); font-weight: 700; letter-spacing: -0.025em; line-height: 1.08; }
  .section p.lead { font-size: clamp(17px, 2vw, 21px); color: var(--ink-soft); max-width: 580px; margin: 16px auto 0; }
  .section-dark p.lead { color: #B9C2D6; }

  /* ---- feature grid ---- */
  .feature-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2px; margin-top: 64px; background: var(--line); border-radius: 24px; overflow: hidden; }
  @media (max-width: 860px) { .feature-grid { grid-template-columns: 1fr; } }
  .feature-cell { background: var(--panel); padding: 40px 32px; text-align: left; }
  .feature-cell .num { font-size: 13px; font-weight: 600; color: var(--gold); margin-bottom: 16px; }
  .feature-cell h3 { font-size: 21px; font-weight: 600; letter-spacing: -0.01em; margin-bottom: 10px; }
  .feature-cell p { font-size: 14.5px; color: var(--ink-soft); line-height: 1.55; }

  /* ---- big stat band ---- */
  .stat-band { display: grid; grid-template-columns: repeat(4, 1fr); gap: 24px; margin-top: 72px; }
  @media (max-width: 760px) { .stat-band { grid-template-columns: 1fr 1fr; } }
  .stat-band .stat .n { font-size: clamp(34px, 4.4vw, 52px); font-weight: 700; letter-spacing: -0.02em; }
  .stat-band .stat .l { font-size: 13.5px; color: var(--ink-soft); margin-top: 6px; }
  .section-dark .stat-band .l { color: #B9C2D6; }

  /* ---- module showcase (cursos) ---- */
  .module-row { display: flex; gap: 16px; overflow-x: auto; margin-top: 56px; padding-bottom: 12px; scroll-snap-type: x mandatory; }
  .module-card {
    flex: 0 0 260px; scroll-snap-align: start; background: var(--panel); border-radius: 22px; padding: 32px 26px;
    text-align: left; text-decoration: none; color: inherit; transition: transform .2s ease;
  }
  .module-card:hover { transform: translateY(-4px); }
  .module-card .icon { width: 44px; height: 44px; border-radius: 12px; background: var(--ink); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 16px; margin-bottom: 20px; }
  .module-card h3 { font-size: 17px; font-weight: 600; margin-bottom: 8px; letter-spacing: -0.01em; }
  .module-card p { font-size: 13.5px; color: var(--ink-soft); line-height: 1.5; }

  /* ---- admin callout ---- */
  .admin-block { display: grid; grid-template-columns: 1fr 1fr; gap: 56px; align-items: center; text-align: left; margin-top: 0; }
  @media (max-width: 860px) { .admin-block { grid-template-columns: 1fr; text-align: center; } }
  .admin-block h2 { font-size: clamp(28px, 4vw, 42px); }
  .admin-block ul { list-style: none; margin-top: 24px; }
  .admin-block li { font-size: 15px; color: #C7C2AE; padding: 10px 0; border-top: 1px solid rgba(255,255,255,0.12); }
  .admin-visual { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.12); border-radius: 22px; padding: 28px; }
  .admin-visual .row { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.08); font-size: 13.5px; }
  .admin-visual .row:last-child { border-bottom: none; }
  .admin-visual .pill { font-size: 11px; padding: 3px 9px; border-radius: 999px; background: rgba(176,141,87,0.22); color: #E9DFC8; }

  /* ---- quote ---- */
  .quote { font-size: clamp(22px, 3vw, 32px); font-weight: 600; letter-spacing: -0.015em; max-width: 760px; margin: 0 auto; line-height: 1.35; }
  .quote-attr { margin-top: 24px; font-size: 14px; color: var(--ink-soft); }

  /* ---- final cta ---- */
  .final { text-align: center; padding: 110px 24px; }
  .final h2 { font-size: clamp(34px, 5vw, 60px); font-weight: 700; letter-spacing: -0.03em; }
  .final p { font-size: 18px; color: var(--ink-soft); margin: 14px 0 32px; }

  footer { border-top: 1px solid var(--line); padding: 32px 24px; text-align: center; font-size: 12px; color: var(--ink-soft); }
...
const LANDING_BODY = `
  ...
  <nav>
    <div class="nav-inner">
      <div class="nav-logo">Dutch Accounting Academy <span style="color:var(--ink-soft); font-weight:400;">for Business</span></div>
      <div class="nav-links">
        <a href="#modulos">Currículo</a>
        <a href="#modulos">Cursos</a>
        <a href="#admin">Modo Empresa</a>
      </div>
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="nav-cta">Entrar →</a>
    </div>
  </nav>

  <section class="hero">
    <div class="eyebrow">Dutch Accounting Academy</div>
    <h1>Formação fiscal e<br/>contabilística. Ao nível<br/>de uma equipa inteira.</h1>
    <p class="sub">Prepara a tua equipa para operar em conformidade com o sistema fiscal neerlandês — do onboarding ao Modo Empresa, com o rigor de uma consultoria.</p>
    <div class="hero-ctas">
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="btn btn-primary">Começar agora</a>
      <a href="#modulos" class="btn btn-ghost">Ver currículo →</a>
    </div>

    <div class="hero-visual">
      <span class="tag">● Ao vivo para a tua equipa</span>
      <div class="stat-row">
        <div class="stat"><div class="n">16</div><div class="l">Módulos técnicos completos</div></div>
        <div class="stat"><div class="n">80+</div><div class="l">Lições com avaliação real</div></div>
        <div class="stat"><div class="n">100%</div><div class="l">Personalizável pela tua equipa</div></div>
      </div>
    </div>
  </section>

  <section class="section" id="modulos">
    <h2>Um currículo.<br/>Todos os módulos que precisas.</h2>
    <p class="lead">De BTW a IFRS — cada módulo combina teoria, prática e avaliação, pronto a usar ou a adaptar à tua empresa.</p>

    <div class="module-row">
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="module-card"><div class="icon">BT</div><h3>BTW</h3><p>IVA neerlandês, reverse-charge, OSS e faturação.</p></a>
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="module-card"><div class="icon">Vp</div><h3>Corporate Tax</h3><p>Vpb, participation exemption, innovatiebox.</p></a>
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="module-card"><div class="icon">Pr</div><h3>Payroll</h3><p>Loonheffing, regra dos 30%, contratos.</p></a>
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="module-card"><div class="icon">GA</div><h3>Dutch GAAP</h3><p>BW2 Titel 9 e demonstrações financeiras.</p></a>
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="module-card"><div class="icon">IF</div><h3>IFRS</h3><p>Conceptual Framework e normas internacionais.</p></a>
      <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="module-card"><div class="icon">ME</div><h3>Modo Empresa</h3><p>Simulações reais, com correção automática.</p></a>
    </div>
  </section>

  <section class="section section-dark" id="admin">
    <div class="wrap">
      <div class="admin-block">
        <div>
          <div class="eyebrow" style="color:#A1A1A6;">Modo Empresa</div>
          <h2>A tua equipa. O teu currículo.</h2>
          <p class="lead" style="margin-left:0;">Cria, edita e organiza cursos, secções e exercícios — sem sair da plataforma, com tradução automática para PT, EN e NL.</p>
          <ul>
            <li>Adicionar e editar cursos completos</li>
            <li>Adicionar e editar secções dentro de qualquer módulo</li>
            <li>Apagar conteúdo a qualquer momento</li>
            <li>Tradução automática entre PT · EN · NL</li>
          </ul>
        </div>
        <div class="admin-visual">
          <div class="row"><span>BTW — IVA Neerlandês</span><span class="pill">10 lições</span></div>
          <div class="row"><span>Onboarding interno (novo)</span><span class="pill">A traduzir…</span></div>
          <div class="row"><span>Corporate Tax (Vpb)</span><span class="pill">6 lições</span></div>
          <div class="row"><span>Procedimentos internos (novo)</span><span class="pill">3 lições</span></div>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="section">
    <div class="stat-band wrap">
      <div class="stat"><div class="n">7</div><div class="l">Simulações no Modo Empresa</div></div>
      <div class="stat"><div class="n">3</div><div class="l">Idiomas nativos</div></div>
      <div class="stat"><div class="n">∞</div><div class="l">Cursos que a tua equipa cria</div></div>
      <div class="stat"><div class="n">0€</div><div class="l">Custo por posto</div></div>
    </div>
  </section>

  <section class="section" id="equipas">
    <p class="quote">"A academia tornou-se o manual de onboarding fiscal da nossa equipa — sem precisarmos de escrever um único documento."</p>
    <div class="quote-attr">Maximiano B. · Fundador, percurso BTW → Vpb</div>
  </section>

  <section class="final">
    <h2>Pronta quando a tua<br/>equipa estiver.</h2>
    <p>Sem custo por posto. Sem cartão de crédito.</p>
    <a href="javascript:void(0)" onclick="window.__daaEnterApp && window.__daaEnterApp()" class="btn btn-primary">Criar conta →</a>
  </section>

  <footer>
    Dutch Accounting Academy for Business · dutch-accounting-academy.vercel.app
  </footer>
...
function LandingPage({ onEnter }) {
  useEffect(() => {
    window.__daaEnterApp = onEnter;
    return () => { delete window.__daaEnterApp; };
  }, [onEnter]);
...
export default function App() {
  const [showLanding, setShowLanding] = useState(() => {
    try { return !window.localStorage.getItem("daa-entered"); } catch { return true; }
  });
  const enterApp = () => {
    try { window.localStorage.setItem("daa-entered", "1"); } catch { /* ignorado */ }
  ...
  const refreshCustomContent = async () => {
    try {
      const [coursesRes, tasksRes, lessonsRes] = await Promise.all([
        api.getCustomCourses().catch(() => ({ courses: [] })),
        api.getCustomTasks().catch(() => ({ tasks: [] })),
        api.getCustomLessons().catch(() => ({ lessons: [] })),
      ]);
      ...
      (lessonsRes.lessons || []).forEach((raw) => {
        const course = COURSES.find((c) => c.id === raw.courseId);
        if (!course) return; // curso pode ter sido apagado entretanto
        const already = course.lessons.some((l) => l.customLessonId === raw.id);
        if (already) return;
        course.lessons.push(normalizeCustomLesson(raw));
      });
  ...
  const NAV = [
    ...
    { key: "account", label: t.nav_account, icon: User },
    { key: "certificates", label: t.nav_certificates, icon: Award },
  ];
  ...
  const Page = useMemo(() => {
    switch (tab) {
      ...
      case "profile": return <ProfilePage t={t} pal={pal} lang={lang} badges={badges} onReset={handleReset} />;
      case "account": return <AccountPage t={t} pal={pal} account={account} />;
      case "certificates": return <CertificatesPage t={t} pal={pal} lang={lang} progress={progress} account={account} />;
  ...
  return (
    <div className="overflow-x-hidden" style={{ background: pal.bg, minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: pal.ink }}>
      <style>{FONTS}</style>
      <div className="flex items-center justify-between px-6 sm:px-10 py-4 sticky top-0 z-20" style={{ background: pal.panel, borderBottom: `1px solid ${pal.border}` }}>
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 flex items-center justify-center flex-shrink-0" style={{ border: `1.5px solid ${pal.ink}` }} aria-hidden>
            <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 12, color: pal.ink }}>DA</span>
          </div>
          <span className="text-sm hidden sm:block" style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, color: pal.ink }}>Dutch Accounting Academy</span>
        </div>
        <div className="flex items-center gap-2.5 text-xs" style={{ color: pal.inkSoft, fontFamily: "'Inter', sans-serif" }}>
          <span>{progress.streak}</span><span style={{ color: pal.muted }}>·</span>
          <span>{progress.coins}</span><span style={{ color: pal.muted }}>·</span>
          {["pt", "en", "nl"].map((l, i) => (
            <React.Fragment key={l}>
              <button onClick={() => setLang(l)} className="uppercase"
                style={{ color: lang === l ? pal.ink : pal.muted, fontWeight: lang === l ? 700 : 400 }}>
                {l}
              </button>
              {i < 2 && <span style={{ color: pal.muted }}>·</span>}
            </React.Fragment>
          ))}
          <span style={{ color: pal.muted }}>·</span>
          <button onClick={() => setTheme(theme === "light" ? "dark" : "light")} style={{ color: pal.inkSoft }}>
            {theme === "light" ? "☾" : "☀"}
          </button>
        </div>
      </div>
      ...
      <div className="flex">
        <div className="hidden md:flex flex-col gap-1 py-10 pl-6 pr-6" style={{ minWidth: 200 }}>
          {NAV.map((n, i) => {
            const active = tab === n.key;
            return (
              <button key={n.key} onClick={() => setTab(n.key)} className="flex items-baseline gap-3 py-1.5 text-sm text-left transition-all"
                style={{
                  color: active ? pal.ink : pal.inkSoft,
                  fontFamily: "'Inter', sans-serif", fontWeight: active ? 600 : 400,
                }}>
                <span style={{ fontSize: 11, color: active ? pal.gold : pal.muted, fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace" }}>
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span style={{ borderBottom: active ? `1px solid ${pal.gold}` : "1px solid transparent", paddingBottom: 2 }}>{n.label}</span>
              </button>
            );
          })}
        </div>
        <div className="flex-1 min-w-0 p-6 sm:p-10 pb-24 md:pb-10" style={{ background: pal.panel, borderTop: `1px solid ${pal.border}`, minHeight: "calc(100vh - 57px)" }}>
          <div className="max-w-3xl">{Page}</div>
        </div>
      </div>
        ...
        {NAV.map((n) => {
          ...
          return (
            <button key={n.key} onClick={() => setTab(n.key)}
              ...
              style={{ color: active ? pal.gold : pal.muted }}
              aria-label={n.label}>
              <Icon size={19} />
              <span className="text-[10px] leading-none font-medium text-center px-0.5"
                style={{ fontFamily: "'Fraunces', serif" }}>{n.label}</span>
            </button>
          );
        })}
      </nav>
